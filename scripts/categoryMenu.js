document.addEventListener('navCargado', () => {
   ...
   const filtrarPorCategoria = (category) => {
      products.forEach((product) => {
         ...
         product.style.display = categoriasProducto.includes(category)
            ? 'block'
            : 'none'
      })
   }
   ...
   navItems.forEach((item) => {
      item.addEventListener('click', function (event) {
         ...
         navItems.forEach((nav) =>
            nav.classList.remove('selected', 'ver-todo-active')
         )
         if (dataCategory === 'VerTodo') {
            activeCategory = 'VerTodo'
            mostrarTodosLosProductos()
            item.classList.add('ver-todo-active')
         } else {
   ...
   banner.addEventListener('click', () => {
      ...
      nav.scrollTo({
         left: verTodoBtn.offsetLeft - nav.offsetLeft,
         behavior: 'smooth',
      })
   })
})
